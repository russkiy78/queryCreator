...
| [`test_qcsqlquery.cpp`](../tests/test_qcsqlquery.cpp) | `QcSqlQuery`: `from`/`join`/`with_`/`where`-chains with `and_`/`or_`/parentheses/`having`/`orderBy`(including `NULLS FIRST`/`LAST`, native and emulated)/`groupBy`/`limit`/set-operations, deep copy, `toSql()` for each construct, end-to-end global placeholder numbering through JOIN-subquery + WHERE + set-operation |
| [`test_qcsqlinsert.cpp`](../tests/test_qcsqlinsert.cpp) | `QcSqlInsert`: `QcVariant` types one per alternative + mixed, large payloads, overwrite-in-place `set()`, `returning()` (both overloads, form and position by driver) |
| [`test_qcsqlupdate.cpp`](../tests/test_qcsqlupdate.cpp) | `QcSqlUpdate`: SET-list, WHERE-chain (via the same `QcSqlQueryElement`), `returning()` |
| [`test_qcsqldelete.cpp`](../tests/test_qcsqldelete.cpp) | `QcSqlDelete`: WHERE-chain, `returning()` |
